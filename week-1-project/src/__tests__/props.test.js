...
test("Renders articles with title ", async () => {
  ...
  const article1 = await findByText(
    "10 Amazing Things you did not know about React"
  );
  const article2 = await findByText("Discover the Best Javascript Framework");
  const article3 = await findByText(
    "This simple trick will teach you everything about React"
  );
...
test("Passing functions as props ", async () => {
  ...
  expect(element.innerHTML).toEqual(
    "Reading all article will take you 1 minutes"
  );
});
...
test("Passing functions as props: part 2", async () => {
  ...
  expect(element.innerHTML).toEqual(
    "Reading all article will take you 2 minutes"
  );
});
...
test("BONUS, I have solved the warning about the unique key", async () => {
  ...
  const iSolvedTheProblem = "I have solved";
